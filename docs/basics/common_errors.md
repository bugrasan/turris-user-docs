...
## Turris MOX not booting from other Turris

Make sure that your Turris MOX does not contain any media to boot from. Also
make sure that your other router has _MOX netboot_ package list enabled.
Last but not least, booting over the network works only via the Ethernet
connector on MOX A and after the first boot, only with the router it is paired
with. To make sure your MOX is not paired with different router, use
[Factory reset](http://turris-docs.myinfra.cz/hw/mox/rescue_modes/#rollback-to-factory-reset).
